...
async function main() {
  ...
  // 3. Deploy MockVerifier (always-true for mock MPC proofs on local anvil)
  console.log("[Deploy] Deploying MockVerifier (MPC proof)...");
  ...
  // 3b. Deploy ClientTransferVerifier (real on-chain ZK proof verification)
  ...
  const privBalanceHash = await walletClient.deployContract({
    ...
    args: [
      ...
      clientVerifierAddress,          // _clientVerifierAddress
  ...
  // ── Write .mpc-balances.json (seed mock MPC with agent's initial balance) ──

  const mpcBalances = {
    randomnessCounter: agentRandomness.toString(),
    balances: {
      [agentAccount.address.toLowerCase()]: {
        balance: agentBalance.toString(),
        randomness: agentRandomness.toString(),
      },
    },
  };
  const mpcBalancesPath = resolve(__dirname, ".mpc-balances.json");
  writeFileSync(mpcBalancesPath, JSON.stringify(mpcBalances, null, 2));
  console.log(`[Seed] MPC balance state written to ${mpcBalancesPath}\n`);
...
# Mock MPC service (for balance checks + transfer hints)
